...
updates_dir=/data/cygnusos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have cygnus-VERSION-DATE-TYPE-* as format"
...
# Assume cygnus-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.cygnusos.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.cygnusos.updater/databases/updates.db" \
